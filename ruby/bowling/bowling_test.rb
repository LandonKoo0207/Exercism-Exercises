...
class BowlingTest < Minitest::Test
  ...
  def test_two_rolls_in_a_frame_cannot_score_more_than_10_points
    ...
    # rolls.each do |pins| 
    #   game.roll(pins)
    #   puts "count: #{game.frames.last.count}, sum: #{game.frames.last.sum + pins}, #{game.frames}, #{pins}"
    # end
    assert_raises Game::BowlingError do
      game.roll(6)
    end
  ...
  def test_cannot_roll_if_game_already_has_ten_frames
    ...
    # rolls.each do |pins|
    #   game.roll(pins)
    #   puts "last_frame: #{game.frames.last}, pins: #{pins}, valid_pins?: #{game.valid_pins?(pins)}, game.game_finished?: #{game.game_finished?(game.frames)}"
    # end
    assert_raises Game::BowlingError do
      game.roll(0)
    end
